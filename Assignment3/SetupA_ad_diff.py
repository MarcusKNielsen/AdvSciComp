...
def scalability(method,order=1):
    ...
    for idx,ki in enumerate(K_list_test):
        A_ad_test = setup_A(12,ki,t,a,d,0,x_left,x_right,method)
...
def setup_A(N,K,t,a,d,alpha,x_left,x_right,method):
    ...
    for i in range(N*K):
        ...
        elif method == diffusion_v2: 
            A[:,i] = method.f_func(t,e,Mk_inv,Dx,S,N,alpha,a,"s")
        elif method == advec_diff_final:
            A[:,i] = method.f_func(t,e,Mk_inv,Dx,S,N,alpha,a,d,"w")
...
Eig_mat_mix = np.zeros([len(N_list),len(K_list)])
Eig_vec_mix = np.zeros(len(N_list)) 

for N_idx,N in enumerate(N_list):
    for K_idx,K in enumerate(K_list):

        A_ad = setup_A(N,K,t,a,d,0,x_left,x_right,advection_v2)
        ...
        A_diff = setup_A(N,K,t,a,d,1,x_left,x_right,diffusion_v2)
        ...
        A_mix = setup_A(N,K,t,a,d,1,x_left,x_right,diffusion_v2)
        eigvals_mix = np.linalg.eigvals(A_mix)
        Eig_mat_mix[N_idx,K_idx] = np.max(np.abs(eigvals_mix))

        if N == K:
            Eig_mat_mix[N_idx] = np.max(np.abs(eigvals_mix))


#scalability(advection_v2)
#scalability(diffusion_v2,order=2)
scalability(advec_diff_final,order=2)
...
for idx,ki in enumerate(K_list_test):
    A_ad_test = setup_A(12,ki,t,a,d,0,x_left,x_right,advection_v2)
    A_diff_test = setup_A(12,ki,t,a,d,0,x_left,x_right,diffusion_v2)
    eigvals_ad_test = np.linalg.eigvals(A_ad_test)
...
plt.figure()
plt.loglog(K_list_test,Eig_vec_test_ad,"--o",label="$\max{|\lambda(\mathcal{A}_{advection})|}$ advection")
# Fit a first-order polynomial (line)
...
plt.loglog(K_list_test,Eig_vec_test_diff,"--o",label="$\max{|\lambda(\mathcal{A}_{diffusion})|}$ diffusion")
# Fit a first-order polynomial (line)
coefficients = np.polyfit(K_list_test, Eig_vec_test_diff, 1)
fit = np.poly1d(coefficients)
#plt.loglog(K_list_test,fit(K_list_test),label="Quadradic fit")
plt.loglog(K_list_test,(K_list_test**2)*10**(3.5),label="$\mathcal{O}(K^2)$ both")
...
plt.figure()
plt.scatter(np.real(eigvals_mix),np.imag(eigvals_mix))
